...
| 持久化写入的确认边界 | 缓冲 I/O 的脏页、回写与确认如何区分？ | [Linux Kernel iomap buffered I/O](https://docs.kernel.org/filesystems/iomap/operations.html) | Linux Kernel Documentation；访问日期：2026-08-30 | Linux 的 buffered I/O 默认使用 page cache；脏缓存随后回写，`fsync` 及其变体可强制回写。 | 不得将 iomap 文档外推为全部文件系统、设备缓存或应用协议的端到端持久化保证。 |
