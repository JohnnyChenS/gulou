...
| 有状态服务恢复 | Kubernetes controller 对 desired/current state 的职责是什么？ | [Kubernetes Controllers](https://kubernetes.io/docs/concepts/architecture/controller/) | Kubernetes Documentation v1.37；访问日期：2026-09-01 | 控制器持续调节当前状态，使其趋近资源所声明的 desired state；可将其作为控制面 reconciliation 的边界说明。 | 不得将控制器观察或 reconciliation 外推为节点上旧进程已停止、应用数据已恢复、分布式一致性已达成、已完成 fencing，或业务 RPO/RTO 保证。 |
| 有状态服务恢复 | StatefulSet 提供哪些身份、顺序和存储编排能力？ | [Kubernetes StatefulSet](https://kubernetes.io/docs/concepts/workloads/controllers/statefulset/) | Kubernetes Documentation v1.37；首批基线访问日期：2026-08-30；复核日期：2026-09-01 | 可引用 StatefulSet 为 Pod 提供稳定标识、稳定网络标识和稳定存储，以及其部署、扩缩与更新行为。 | 不得把控制器编排外推为应用层复制、数据一致性、自动故障恢复或业务 RPO/RTO 保证。 |
| 有状态服务恢复 | PersistentVolume 与 PersistentVolumeClaim 解决了什么边界？ | [Kubernetes Persistent Volumes](https://kubernetes.io/docs/concepts/storage/persistent-volumes/) | Kubernetes Documentation v1.37；首批基线访问日期：2026-08-30；复核日期：2026-09-01 | 可引用 PV/PVC 的生命周期、绑定和存储资源抽象。 | 不得据此宣称卷本身保证数据库崩溃一致性、跨区域复制、备份、加密或任意存储后端的性能。 |
| 有状态服务恢复 | 为什么 StatefulSet Pod 的强制删除不是常规恢复手段？ | [Kubernetes force-delete StatefulSet Pod](https://kubernetes.io/docs/tasks/run-application/force-delete-stateful-set-pod/) | Kubernetes Documentation v1.37；首批基线访问日期：2026-08-30；复核日期：2026-09-01 | 可引用强制删除的操作语义和官方风险警告，用于说明人工核验、隔离与应用层恢复的必要性。 | 不得把强制删除描述为安全的默认自愈操作，或据此推断应用数据不会损坏、不会出现双实例或已经完成业务恢复。 |
