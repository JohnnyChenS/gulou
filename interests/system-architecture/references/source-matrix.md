...
| 分区复制与提交边界 | `min.insync.replicas` 与 unclean leader election 如何改变 Kafka 4.2 的可用性/耐久取舍？ | [Apache Kafka 4.2 topic configuration](https://kafka.apache.org/42/configuration/topic-configs/#min.insync.replicas)；[unclean.leader.election.enable](https://kafka.apache.org/42/configuration/topic-configs/#unclean.leader.election.enable) | Apache Kafka 4.2；访问日期：2026-08-30 | 对 `acks=all`，`min.insync.replicas` 限制写入成功所需的最小 ISR 数；当前 ISR 小于该值时生产者得到 `NotEnoughReplicas` 或 `NotEnoughReplicasAfterAppend`。`acks=all` 仍等待全部当前 ISR，而非只等待最小值。允许非 ISR 副本作为最后手段被选作 leader 的 unclean leader election 可能导致数据丢失，默认值为 `false`。Kafka 4.2 还注明启用 Eligible Leader Replicas（ELR）时该配置语义会变化。 | 不得把 `min.insync.replicas` 单独用于推断 `acks=0` 或 `acks=1` 的确认语义，或宣称启用 unclean election 后仍保留此前已确认记录；不得把默认值替代实际 topic/broker 配置核验。本矩阵未经 ELR 配置核验时，不得套用至启用 ELR 的 topic。 |
